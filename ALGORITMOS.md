...
**Implementação vetorizada:** em vez de um loop Python pixel a pixel, criamos arrays com todas as coordenadas de destino de uma vez (`np.arange`), calculamos as coordenadas de origem correspondentes com operações de array, e usamos `np.meshgrid` pra montar uma grade 2D com todas as combinações de coordenadas. Indexamos a imagem direto com essas grades pra pegar todos os pixels numa operação só. Numpy executa isso internamente em C, então é ~100x mais rápido que loops Python.
...
**Implementação vetorizada:** usa `np.meshgrid` pra montar grades 2D com todas as coordenadas fracionárias de uma vez. A partir dessas grades calcula os índices dos 4 vizinhos e os pesos (dx, dy). A média ponderada é uma expressão numpy que opera em todos os pixels simultaneamente — pra imagens coloridas, adiciona uma dimensão extra nos pesos com `np.newaxis` pra se combinar com os 3 canais.
